...
use crate::storage::{ThreadData, ThreadMetadata, ThreadStorage, DEFAULT_THREAD_TITLE};
use chrono::Utc;
use serde::Serialize;
...
fn create_thread_on_disk(
    ...
    match workspace_id
        .map(str::trim)
        .filter(|id| !id.is_empty() && *id != "recents")
    {
...
async fn run_brain_job(
    ...
    let result = async {
        ...
        let model_candidates = brain()
            .build_model_attempt_plan(model, effort, "micro".to_string())
            .await?;
        let title = brain()
