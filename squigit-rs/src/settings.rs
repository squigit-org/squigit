...
const VALID_WORKSPACE_ORDERINGS: &[&str] = &["created", "updated"];
const VALID_THREAD_ORDERINGS: &[&str] = &["created", "updated"];
...
pub struct DesktopConfig {
    ...
    pub workspace_ordering: String,
    pub thread_ordering: String,
}
...
pub struct DesktopConfigUpdate {
    ...
    pub workspace_ordering: Option<String>,
    pub thread_ordering: Option<String>,
}
...
impl Default for DesktopConfig {
    fn default() -> Self {
        Self {
            ...
            show_tray_icon: true,
            workspace_ordering: "updated".to_string(),
            thread_ordering: "updated".to_string(),
        }
    }
}
...
fn default_config_table() -> toml::Table {
    ...
    desktop.insert(
        "workspace_ordering".to_string(),
        toml::Value::String(desktop_defaults.workspace_ordering),
    );
    desktop.insert(
        "thread_ordering".to_string(),
        toml::Value::String(desktop_defaults.thread_ordering),
    );
...
fn normalize_desktop_config(table: &toml::Table) -> (DesktopConfig, bool) {
    ...
    let capture_type = desktop_table
        .get("capture_type")
        .or_else(|| desktop_table.get("captureType"))
        .and_then(|v| v.as_str())
    ...
    let show_tray_icon = desktop_table
        .get("show_tray_icon")
        .or_else(|| desktop_table.get("showTrayIcon"))
        .and_then(|v| v.as_bool())
        .unwrap_or_else(|| {
            modified = true;
    ...
    let workspace_ordering = desktop_table
        .get("workspace_ordering")
        .or_else(|| desktop_table.get("workspaceOrdering"))
        .and_then(|v| v.as_str())
        .filter(|o| VALID_WORKSPACE_ORDERINGS.contains(o))
        .map(|s| s.to_string())
        .unwrap_or_else(|| {
            modified = true;
            defaults.workspace_ordering
        });

    let thread_ordering = desktop_table
        .get("thread_ordering")
        .or_else(|| desktop_table.get("threadOrdering"))
        .and_then(|v| v.as_str())
        .filter(|o| VALID_THREAD_ORDERINGS.contains(o))
        .map(|s| s.to_string())
        .unwrap_or_else(|| {
            modified = true;
            defaults.thread_ordering
        });
...
fn set_desktop_table(table: &mut toml::Table, config: &DesktopConfig) {
    ...
    desktop.insert(
        "workspace_ordering".to_string(),
        toml::Value::String(config.workspace_ordering.clone()),
    );
    desktop.insert(
        "thread_ordering".to_string(),
        toml::Value::String(config.thread_ordering.clone()),
    );
    table.insert("desktop".to_string(), toml::Value::Table(desktop));
}
...
pub fn update_desktop_config(updates: DesktopConfigUpdate) -> SettingsResult<DesktopConfig> {
    ...
    let next_workspace_ordering = updates
        .workspace_ordering
        .filter(|o| VALID_WORKSPACE_ORDERINGS.contains(&o.as_str()))
        .unwrap_or(current.workspace_ordering);
    let next_thread_ordering = updates
        .thread_ordering
        .filter(|o| VALID_THREAD_ORDERINGS.contains(&o.as_str()))
        .unwrap_or(current.thread_ordering);

    let next = DesktopConfig {
        theme: next_theme,
        capture_type: next_capture_type,
        show_tray_icon: next_show_tray_icon,
        workspace_ordering: next_workspace_ordering,
        thread_ordering: next_thread_ordering,
    };
...
pub fn reveal_api_key(
    ...
    let store = ProfileStore::new().map_err(|error| error.to_string())?;
    let authorization = check_reveal_authorization(&store, RevealShell::Electron)
        .map_err(|error| error.to_string())?;
