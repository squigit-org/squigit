...
| **Container**    | Electron         | `packages/snapllm/package.json`    | JSON   |
...
SnapLLM implements a "self-healing" update mechanism. The application does not download update binaries directly; instead, it delegates the update process to the resident **Bootstrap Installer**.
...
1. **Locator:** The application resolves the path to the permanent installer binary (`snapllm-installer`) located in the parallel `../snapllm-installer/` directory.
2. **Execution:**
...
2. Overwrites the contents of `~/.local/share/snapllm` (or equivalent).
