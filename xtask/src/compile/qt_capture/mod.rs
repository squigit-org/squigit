...
fn build(native_dir: &Path) -> Result<()> {
    #[cfg(target_os = "linux")]
    return linux::build(native_dir);
    #[cfg(target_os = "macos")]
    return macos::build(native_dir);
    #[cfg(target_os = "windows")]
    return win::build(native_dir);
}

fn deploy(native_dir: &Path) -> Result<()> {
    #[cfg(target_os = "linux")]
    return linux::deploy(native_dir);
    #[cfg(target_os = "macos")]
    return macos::deploy(native_dir);
    #[cfg(target_os = "windows")]
    return win::deploy(native_dir);
}
