## SnapLLM — AI assistant instructions
...
- `packages/snapllm` — Electron app shell and app-specific srcs in `src/`. This package depends on `core/dist` and contains renderer, preload and main process code.
...
- The Electron app in `packages/snapllm` embeds the UI from `packages/core/dist` by copying it into `src/renderer/view` during the build. See `setup.py` function `build_snapllm()` for the exact steps.
- The Electron main process uses IPC handlers in `packages/snapllm/src/ipc-handlers/` (example: `dialogs.js`) to communicate with renderer code. Changes to IPC handlers must be reflected in the renderer `ipcRenderer` calls.
...
- Full orchestrated build (CI-style): `python3 setup.py` (this script orchestrates setting permissions, building capturekit, building orchestrator, building snapllm, and running `pytest`).
- Manual frontend build:
  - `cd packages/core && npm ci && npm run build`
  - `cd packages/snapllm && npm ci && npm run build:linux|mac|win` (scripts follow `setup.py` naming: `build:linux`, `build:mac`, `build:win`)
...
- The Electron app copies `core/dist` into `packages/snapllm/src/renderer/view`. Do not edit generated files in that view directory; change src in `packages/core` and rebuild.
- IPC handlers live in `packages/snapllm/src/ipc-handlers/` and export a function that accepts `ipcMain`. The handler files use `ipcMain.handle(...)` and return an object with higher-level helpers. When adding new IPC channels, update both handler and renderer-side callers in `packages/snapllm/src/renderer/`.
...
- Node.js & npm (or pnpm/yarn as the project uses npm by default). `packages/core` and `packages/snapllm` must be built with compatible Node versions.
...
- Updating UI shared code: edit `packages/core/src/*`, then run `cd packages/core && npm ci && npm run build`. After successful build, run `cd packages/snapllm && python3 ../../setup.py` or run the `build_snapllm` steps in `setup.py` manually.
- Adding an IPC channel: add a handler `ipcMain.handle('my-channel', ...)` in `packages/snapllm/src/ipc-handlers/yourfile.js`, then call it from renderer via `ipcRenderer.invoke('my-channel', args)` and add tests in `tests/unit` that mock `ipcMain` or test the renderer call using Jest/Electron-mocha as appropriate.
...
- `packages/snapllm/src/` — Electron app src, IPC handlers, renderer/main/preload
