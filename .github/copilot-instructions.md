...
- `packages/spatialshot` — Electron app shell and app-specific sources in `source/`. This package depends on `core/dist` and contains renderer, preload and main process code.
...
- The Electron app in `packages/spatialshot` embeds the UI from `packages/core/dist` by copying it into `source/renderer/view` during the build. See `setup.py` function `build_spatialshot()` for the exact steps.
- The Electron main process uses IPC handlers in `packages/spatialshot/source/ipc-handlers/` (example: `dialogs.js`) to communicate with renderer code. Changes to IPC handlers must be reflected in the renderer `ipcRenderer` calls.
...
- The Electron app copies `core/dist` into `packages/spatialshot/source/renderer/view`. Do not edit generated files in that view directory; change source in `packages/core` and rebuild.
- IPC handlers live in `packages/spatialshot/source/ipc-handlers/` and export a function that accepts `ipcMain`. The handler files use `ipcMain.handle(...)` and return an object with higher-level helpers. When adding new IPC channels, update both handler and renderer-side callers in `packages/spatialshot/source/renderer/`.
...
- Adding an IPC channel: add a handler `ipcMain.handle('my-channel', ...)` in `packages/spatialshot/source/ipc-handlers/yourfile.js`, then call it from renderer via `ipcRenderer.invoke('my-channel', args)` and add tests in `tests/unit` that mock `ipcMain` or test the renderer call using Jest/Electron-mocha as appropriate.
...
- Keep changes minimal and localized. Respect the generated `view/` directory policy: edit `core` sources, not `core/dist` copies.
...
- `packages/core/` — shared frontend source and build scripts
- `packages/spatialshot/source/` — Electron app source, IPC handlers, renderer/main/preload
