...
import { type DialogContent, getMissingPackageDialog } from "@squigit/core/helpers";
...
export const VoiceButton: React.FC<VoiceButtonProps> = ({
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      try {
        const unlisten = await platformBridge.listen<SttEvent>("stt_event", (payload) => {
  ...
  const toggleRecording = useCallback(async () => {
    ...
    if (isRecording) {
      try {
        await platformBridge.invoke("stop_stt");
      } catch (err) {
      ...
      try {
        setIsRecording(true);
        await platformBridge.invoke("start_stt", {
        ...
        if (errorText.includes("ERR_MISSING_STT_PACKAGE")) {
          setErrorDialog(
            getMissingPackageDialog(
              "squigit-stt",
              platformHooks.getPkgInstallCmd("squigit-stt"),
            ),
          );
        } else {
