...
import { OcrModel, AVAILABLE_MODELS } from "../ocr-models.types";
...
export const getInstalledModelIds = async (): Promise<string[]> => {
  try {
    const models = await platform.invoke<string[]>("list_downloaded_models");
    return models;
  } catch (error) {
...
export const getModelPath = async (modelId: string): Promise<string | null> => {
  try {
    const path = await platform.invoke<string>("get_model_path", { modelId });
    return path;
  } catch (error) {
