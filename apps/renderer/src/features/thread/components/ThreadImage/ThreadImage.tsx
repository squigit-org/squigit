...
import { generateTranslateUrl } from "@squigit/core/services/google";
...
export const ThreadImage: React.FC<ThreadImageProps> = ({
  ...
  useEffect(() => {
    ...
    const runToneDetection = async () => {
      ...
      while (attempt < 3 && !cancelled) {
        try {
          const raw = await platformBridge.invoke<string>("detect_image_tone", {
          ...
          if (!cancelled) {
            ...
            if (threadId) {
              saveImageTone(threadId, tone).catch(console.error);
            }
            console.log(
  ...
  const imageSrc = startupImage?.path
    ? platformBridge.convertFileSrc(startupImage.path)
    : "";
  ...
  }, [threadId, startupImage?.imageId, startupImage?.path]);
  useEffect(() => {
  ...
  const isScanContextCurrent = useCallback(
    (
      context: { threadId: string | null; imageId: string; imagePath: string },
      requestId?: number,
    ) => {
      ...
      return (
        latest.threadId === context.threadId &&
  ...
  const scanContextKey = `${threadId ?? "__none__"}::${startupImage?.imageId ?? "__none__"}::${startupImage?.path ?? "__none__"}`;
  useEffect(() => {
  ...
  const scan = useCallback(
    async (
      ...
      if (hasScannedOcrEntry(ocrData[modelToUse]) && !options?.force) {
        console.log(
          `[ThreadImage] Data already exists for model: ${modelToUse}`,
        );
        return;
      }

      const lockKey = `${scanContext.threadId ?? "__no_thread__"}-${scanContext.imageId}-${modelToUse}`;
      if (globalScanLock.has(lockKey)) {
        console.log(`[ThreadImage] Scan already in progress for: ${lockKey}`);
        return;
      }
      ...
      console.log(
        `[ThreadImage] Scanning with model: ${modelToUse} (Request ID: ${requestId})`,
      );
      setLoading(true);
      setErrorDialog(null);
      ...
      try {
        ...
        if (!isScanContextCurrent(scanContext, requestId)) {
          console.log(
            `[ThreadImage] Ignoring stale OCR result for ${modelToUse}`,
          );
          return;
        }
        ...
        console.log(`[ThreadImage] Updating OCR data for ${modelToUse}`);
        onUpdateOCRData(scanContext.threadId, modelToUse, converted);
  ...
  const handleCopyImage = useCallback(async (): Promise<boolean> => {
    ...
    try {
      await platformBridge.invoke("copy_image_from_path_to_clipboard", {
        path: startupImage.path,
      });
      return true;
    } catch (err) {
  ...
  const handleExpandSave = useCallback(async () => {
    ...
    try {
      const filePath = await platformBridge.dialog.save({
        filters: [
      ...
      if (filePath) {
        await platformBridge.invoke("copy_image_to_path", {
