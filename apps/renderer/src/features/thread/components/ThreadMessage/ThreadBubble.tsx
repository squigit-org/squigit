...
> {
  ...
  componentDidCatch(error: unknown, info: ErrorInfo) {
    console.error("[thread] Markdown render failed:", error, info);
  }
...
function resolveCitationFaviconSources(
  ...
  const fallbackSrc =
    primary === fallback ? undefined : faviconUrl?.trim() || fallback;
...
const ThreadBubbleComponent: React.FC<ThreadBubbleProps> = ({
  ...
  const handleImageClick = useCallback(
    ...
    [threadId, imageAttachments, openMediaViewer],
  );
...
export const ThreadBubble = React.memo(
  ThreadBubbleComponent,
  (prevProps, nextProps) => {
    return (
