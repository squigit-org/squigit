    ...
    try {
      const result = await platform.dialog.open({
        ...
        filters: [
          {
            name: "Source Files",
            extensions: ["js", "ts", "jsx", "tsx", "py", "rs", "go", "json", "md", "html", "css", "c", "cpp", "h", "hpp", "java", "sh", "txt"],
          },
          {
            name: "All Files",
            extensions: ["*"],
      ...
      const isText = await commands.validateTextFile(selectedPath);
      if (!isText) {
        console.warn(
      ...
      const importedText = normalizeCode(await platform.fs.readTextFile(selectedPath));
