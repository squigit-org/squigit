...
export interface ThreadInputEditorHandle {
  focus: () => void;
  blur: () => void;
  resetScroll: () => void;
...
const Placeholder = Extension.create<{
  ...
  addProseMirrorPlugins() {
    return [
      new Plugin({
        key: new PluginKey("threadInputPlaceholder"),
        props: {
          decorations: (state) => {
            const { doc } = state;
...
async function readClipboardImagePath(): Promise<string | null> {
  try {
    const result = await navigator.clipboard
      .read()
      .then((items) => items[0]?.getType("image/png"));
    if (!result) return null;
    const { path } = await commands.storeImageBytes(
      Array.from(new Uint8Array(await result.arrayBuffer())),
      "Pasted Image",
    );
    return path || null;
  } catch {
...
async function readClipboardTextValue(): Promise<string> {
  try {
    return (await commands.readClipboardText()) || "";
  } catch {
    try {
      ...
      <>
        ...
        <div className={styles.inputArea}>
          <div
            ...
            style={
              {
                "--thread-input-max-height": maxHeight,
