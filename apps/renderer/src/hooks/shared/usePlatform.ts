...
import { commands, platformType } from "@/platform";
...
export function usePlatform(): PlatformInfo {
  ...
  const base = useMemo(() => {
    const os = platformType as Platform;
  ...
  useEffect(() => {
    const loadLinuxPackageManager = async () => {
      if (base.os !== "linux") return;
      try {
        const pm = await commands.getLinuxPackageManager();
        setLinuxDistro(pm as "debian" | "rpm" | "unknown");
      } catch (err) {
        console.error("Failed to get linux package manager:", err);
      }
    };

    loadLinuxPackageManager();
  }, [base.os]);
