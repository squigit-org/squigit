...
export const ThreadRoute: React.FC = () => {
  ...
  const scrollFollowStateRef = useRef({
    ...
    isRevealPendingForActiveThread: false,
  });
  ...
  const latestCommittedMessageCountRef = useRef(app.thread.messages.length);

  // Hooks
  const error = app.thread.error || app.system.systemError;
  const { isErrorOpen, parsedError, errorActions } = useThreadError(
    error,
    app.system.openSettings,
  );
  const { isImageExpanded, setIsImageExpanded } = useThreadWheel(
    headerRef,
    app.threadHistory.activeSessionId,
  );

  const { inputContainerRef, inputHeight } = useThreadInputHeight();
  ...
  const isNavigationLoading = app.isNavigating || !app.isThreadContentReady;
  const showArtifactPlaceholder = isNavigationLoading && !visibleStartupImage;

  const revealTarget = app.searchOverlay.pendingReveal;
  const isRevealPendingForActiveThread =
    revealTarget?.threadId === app.threadHistory.activeSessionId;

  const { isSpinnerVisible, isAtBottom } = useThreadScroll({
  ...
  const [isContentMounted, setIsContentMounted] =
    useState(!isNavigationLoading);
  const deferredMessages = useDeferredValue(app.thread.messages);
  const [pendingAssistantTurnBridge, setPendingAssistantTurnBridge] = useState(
    app.thread.pendingAssistantTurn,
  );
  const lastPendingAssistantTurnRef = useRef(app.thread.pendingAssistantTurn);
  const effectivePendingAssistantTurn =
    app.thread.pendingAssistantTurn ?? pendingAssistantTurnBridge;
  const shouldUseDeferredMessages =
    !!effectivePendingAssistantTurn &&
    deferredMessages.length <= app.thread.messages.length &&
    deferredMessages.every(
      (message, index) => message.id === app.thread.messages[index]?.id,
    );
  ...
  const hasOlderHiddenMessages =
    app.thread.messages.length > visibleWindowedMessages.length;
  const showScrollToBottomButton =
    !isSpinnerVisible &&
    !showLoadingOverlay &&
    !isRevealPendingForActiveThread &&
    !isAtBottom;

  scrollFollowStateRef.current = {
    hasPendingTurn: !!app.thread.pendingAssistantTurn,
  ...
  const handleScrollToBottom = useCallback(() => {
    wasAtBottomRef.current = true;
    setIsStreamingAutoScrollEnabled(true);
    if (app.thread.pendingAssistantTurn) {
      scrollBottomIntoView("auto");
      return;
    }
  ...
  const handlePendingTurnLayoutChange = useCallback(() => {
    const {
      ...
      isRevealPendingForActiveThread: revealPending,
    } = scrollFollowStateRef.current;
  ...
  useEffect(() => {
    latestCommittedMessageCountRef.current = app.thread.messages.length;
  }, [app.thread.messages.length]);
  ...
  useEffect(() => {
    const livePendingTurn = app.thread.pendingAssistantTurn;
    ...
    const hasCommittedTurn = app.thread.messages.some(
  ...
  }, [app.thread.messages, app.thread.pendingAssistantTurn]);
  ...
  useEffect(() => {
    const currentMessages = app.thread.messages;
  ...
  useLayoutEffect(() => {
    ...
    if (isRevealPendingForActiveThread) {
  ...
  useLayoutEffect(() => {
    const shouldFollowStreaming =
      !!app.thread.pendingAssistantTurn && isStreamingAutoScrollEnabled;
    const shouldKeepPinnedForNonStreaming =
      !app.thread.pendingAssistantTurn && wasAtBottomRef.current;
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (!isStreamingAutoScrollEnabled || !app.thread.pendingAssistantTurn) {
        return;
      }
      if (isProgrammaticScrollRef.current) {
        return;
  ...
  useEffect(() => {
    ...
    setShowLoadingOverlay(true);
    setIsContentMounted(false);
  }, [app.threadHistory.activeSessionId, isNavigationLoading]);

  const isDeferredContentReady = deferredMessages === app.thread.messages;
  ...
  // Capture listen
  useEffect(() => {
    const unlistenPromise = platform.listen<{ tempPath: string }>(
      "capture-to-input",
      (payload) => {
        if (payload && payload.tempPath) {
          app.addAttachmentFromPath(payload.tempPath);
        }
      },
    );
    return () => {
  ...
  const handleSelectAll = () => {
    ...
    const bubble = element?.closest('[data-component="thread-bubble"]');
  ...
  const handleCaptureToInput = useCallback(async () => {
    try {
      await platform.invoke("spawn_capture_to_input");
    } catch (err) {
  ...
  const getRetryAttachments = useCallback(
    (messageId: string) => {
      const targetIndex = app.thread.messages.findIndex(
      ...
      const sourceUserMessage = app.thread.messages
        .slice(0, targetIndex)
        .reverse()
    ...
    [app.thread.messages, app.system.startupImage],
  );
  ...
  const handleUndoDialogAction = useCallback(
    (actionKey: string) => {
      ...
      const targetMessage = app.thread.messages.find(
      ...
      app.thread.handleUndoMessage(messageId);
    },
  ...
  useEffect(() => {
    ...
    if (!target) return;
    if (app.threadHistory.activeSessionId !== target.threadId) return;
    ...
    const revealBubble = (): boolean => {
      ...
      const selector = `[data-component="thread-bubble"][data-message-index="${target.messageIndex}"]`;
  ...
  const isImageProgressVisible =
    !!app.system.startupImage &&
    app.thread.messages.length === 0 &&
    app.thread.isAnalyzing;
  const imageProgressText = getVisibleImageProgressText(app.thread.toolStatus);
  const isInitialRetryTurn =
    app.thread.pendingAssistantTurn?.requestKind === "initial" &&
    !!app.thread.retryingMessageId;
  const delayedImageAttachmentProgressText = getAttachmentAnalysisStatusText(
    app.pendingPromptAttachmentAnalysis,
  );

  useEffect(() => {
    const turnId = app.thread.pendingAssistantTurn?.id ?? null;
    ...
    app.thread.pendingAssistantTurn?.id,
    imageProgressText,
  ...
  useEffect(() => {
    const turnId = app.thread.pendingAssistantTurn?.id;
  ...
  const visibleImageProgressText = imageProgressText
    ...
    : !isInitialRetryTurn &&
      ...
      : null;
  const hasRunningToolStep = app.thread.streamingToolSteps.some(
    (step) => step.status === "running",
  );
  const hasPreStepSearchStatus =
    !!app.thread.toolStatus &&
    app.thread.streamingToolSteps.length === 0 &&
    app.thread.isSearching;
  ...
  return (
    <ThreadWorkspace
      ...
      threadId={app.threadHistory.activeSessionId}
