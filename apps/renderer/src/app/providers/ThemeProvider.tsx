...
import { commands } from "@/platform";import { loadPreferences, savePreferences } from "@squigit/core/config";
...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const applyDomTheme = useCallback((targetTheme: "light" | "dark") => {
    ...
    commands.setBackgroundColor(bgColor).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    const setupTheme = async () => {
      if (theme === "system") {
        try {
          const sysTheme = await commands.getSystemTheme();
