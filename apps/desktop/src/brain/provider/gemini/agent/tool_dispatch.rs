...
pub(crate) struct WebToolDispatchState {
    pub(crate) allowed_sources: HashMap<String, crate::brain::tools::web::CitationSource>,
...
async fn execute_local_attachment_context(
    ...
    let bounded_max =
        crate::brain::provider::gemini::attachments::clamp_tool_max_chars(requested_max);

    let result = crate::brain::provider::gemini::attachments::read_local_attachment_context(
        path,
        Some(bounded_max),
    )
    .await;
    ...
    let (status, message, is_failure) = match &result {
        crate::brain::provider::gemini::attachments::LocalAttachmentContextResult::Success(
            success,
        ) => {
        ...
        crate::brain::provider::gemini::attachments::LocalAttachmentContextResult::Failure(
            failure,
        ) => (
            "error".to_string(),
                ...
                if resolved.is_none() {
                    ...
                    if resolved.is_none() {
                        let filtered_candidates =
                            crate::brain::tools::web::filter_suggested_urls_to_safe_sources(
                                &suggested_urls,
                                &context.web_state.attempted_domains,
                                3,
                            );
