...
pub fn resolve_sidecar_path(_app: &AppHandle) -> Result<(PathBuf, Option<PathBuf>), String> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if brew_arm.exists() { return Ok((brew_arm, None)); }
        if brew_intel.exists() { return Ok((brew_intel, None)); }
    }
    ...
    #[cfg(target_os = "windows")]
    {
        if let Ok(local_app_data) = std::env::var("LOCALAPPDATA") {
            let winget_path = std::path::PathBuf::from(local_app_data)
                .join("Microsoft").join("WindowsApps").join("squigit-stt.exe");
            if winget_path.exists() { return Ok((winget_path, None)); }
        }
    }
    ...
    if let Ok(current_exe) = std::env::current_exe() {
        if let Some(target_dir) = current_exe.parent().and_then(|p| p.parent()) {
            let host_triple = if cfg!(target_os = "windows") { "x86_64-pc-windows-msvc" } 
                else if cfg!(target_os = "macos") { "aarch64-apple-darwin" } 
                else { "x86_64-unknown-linux-gnu" };
            
            let debug_runtime = target_dir.join("debug").join("binaries").join(format!("whisper-stt-{}", host_triple));
...
fn check_stt_version(sidecar_path: &std::path::Path) -> Result<(), String> {
    ...
    let req: Vec<u32> = REQUIRED_STT_VERSION.split('.').filter_map(|s| s.parse().ok()).collect();
