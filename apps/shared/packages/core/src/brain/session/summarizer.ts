 ...
 *
 * Inspired by OpenAI Codex's compaction architecture but adapted for thread UX.
 */
 ...
 *
 * @param threadId - Active thread ID for persisting the summary
 */
export function maybeCompressHistory(threadId: string | null): void {
  const history = brainSessionStore.conversationHistory;
  ...
  compressConversationHistory(
    ...
    .catch((e) => {
      ...
      // The old history stays, just slightly over the ideal context size.
      console.warn("[Summarizer] Compression failed (non-fatal):", e);
    });
}

/**
 * Set the conversation summary directly (used when restoring from saved thread).
 */
