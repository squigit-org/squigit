...
impl BrainService {
    ...
    pub async fn stream_thread(
        ...
        let credential = tokio::task::spawn_blocking(move || {
            ...
            squigit_auth::get_decrypted_api_key(
                &store,
        ...
        let credential = crate::provider::gemini::attachments::ActiveCredential::new(credential);
    ...
    /// Resolve the active Google credential and ensure one canonical CAS image
    /// has a valid Gemini Files remote for that credential.
    pub async fn ensure_thread_image_uploaded(
        &self,
        image_path: String,
    ) -> Result<GeminiFileRef, String> {
        let api_key = crate::provider::gemini::attachments::load_active_api_key().await?;
        crate::provider::gemini::attachments::ensure_file_uploaded(
            &self.runtime,
            &api_key,
            &image_path,
            None,
    ...
    /// Generate an initial thread title from an already-resolved Gemini file.
    /// This deliberately reuses the remote produced by
    /// [`Self::ensure_thread_image_uploaded`].
    pub async fn suggest_thread_title_from_file(
        &self,
        file: GeminiFileRef,
        model_candidates: Vec<String>,
    ) -> Result<String, String> {
    ...
    pub async fn analyze_image(
        ...
        let title_thread_id = metadata.id.clone();
        let title_candidates = request.micro_model_candidates;
        let (assistant_result, title_result) = tokio::join!(
            self.stream_thread(
                sink,
                StreamThreadRequest {
                    profile_id: request.profile_id,
                    model_candidates: request.main_model_candidates,
            ...
            self.suggest_thread_title(title_thread_id, title_candidates),
        );

        match title_result {
            Ok(title) if !title.trim().is_empty() => {
                let mut generated_metadata = storage
                    .load_thread(&metadata.id)
                    .map_err(|error| error.to_string())?
                    .metadata;
                generated_metadata.title = title.trim().to_string();
                storage
                    .update_thread_metadata(&generated_metadata)
                    .map_err(|error| error.to_string())?;
            }
            Ok(_) => eprintln!("[ThreadTitle] Title generation returned an empty title"),
            Err(error) => eprintln!("[ThreadTitle] Title generation failed: {error}"),
        }

        let assistant_message = assistant_result?;
