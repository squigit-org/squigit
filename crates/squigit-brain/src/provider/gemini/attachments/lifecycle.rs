...
pub(crate) struct AttachmentPreflightLease {
    ...
    pub(crate) identity: ActiveKeyIdentity,
...
async fn run_shared_upload(
    ...
    let work_key = identity.object_remote_id(hash)?;
    let mut receiver = {
        let mut work = runtime.attachment_work.lock().await;
        ...
        let prepared = match prepared {
            ...
            Err(error) => {
                ...
                match load_active_key_identity() {
                    Ok(latest) if !latest.matches(&identity) => continue,
...
async fn wait_for_connectivity(
    ...
    loop {
        if !load_active_key_identity()?.matches(identity) {
            return Err("KEY_CHANGED".to_string());
        }
        let response = tokio::select! {
            result = client
                .get("https://generativelanguage.googleapis.com/v1beta/models")
                .header("x-goog-api-key", identity.api_key())
                .send() => result,
            _ = cancellation.cancelled() => return Err("CANCELLED".to_string()),
        };
        match response {
...
mod tests {
    ...
    fn lease() -> AttachmentPreflightLease {
        AttachmentPreflightLease {
            ...
            identity: ActiveKeyIdentity::for_test("test-google-api-key"),
    ...
    async fn preflight_lease_is_bound_and_consumed_once() {
        ...
        assert_eq!(consumed.identity.api_key(), "test-google-api-key");
