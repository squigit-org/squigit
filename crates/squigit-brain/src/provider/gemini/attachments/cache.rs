...
impl std::fmt::Debug for ActiveKeyIdentity {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("ActiveKeyIdentity([REDACTED])")
    }
}

impl ActiveKeyIdentity {
    ...
    pub(crate) fn object_remote_id(&self, object_hash: &str) -> Result<String, String> {
        object_remote_id(
...
pub(crate) fn load_active_key_identity() -> Result<ActiveKeyIdentity, String> {
    ...
    Ok(ActiveKeyIdentity::new(credential))
}

pub(crate) fn load_active_api_key() -> Result<String, String> {
    Ok(load_active_key_identity()?.api_key().to_owned())
}

pub(crate) fn active_key_identity(api_key: &str) -> Result<ActiveKeyIdentity, String> {
    let identity = load_active_key_identity()?;
    if identity.api_key() != api_key.trim() {
...
pub(crate) async fn ensure_file_uploaded_for_identity(
    ...
    let canonical_path = storage
        ...
        .map_err(|error| error.to_string())?;
    let remote_id = identity.object_remote_id(&hash)?;
