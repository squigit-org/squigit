...
use crate::provider::gemini::attachments::{
    ...
    prepare_turn_attachments, ActiveCredential,
};
...
async fn stream_gemini_thread_candidate(
    ...
    let api_key = credential.api_key().to_owned();
    let result = async {
        ...
        let mut contents: Vec<GeminiContent> = if is_initial_turn {
            ...
            if let Some(path) = image_path.clone() {
                let file_ref = crate::provider::gemini::attachments::ensure_file_uploaded_for_credential(
                    runtime,
                    &credential,
                    &path,
