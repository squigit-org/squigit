...
impl TransportClients {
    ...
    pub(crate) fn route_order(&self) -> Vec<TransportRoute> {
        if self.proxy.is_some() {
            vec![TransportRoute::Proxy, TransportRoute::Direct]
        } else {
            vec![TransportRoute::Direct]
        }
    }
    ...
    F: FnMut(&reqwest::Client) -> reqwest::RequestBuilder,
{
    ...
    for (idx, route) in routes.iter().copied().enumerate() {
        ...
        match request_factory(client).send().await {
            ...
            {
                log::debug!(
                    "[WebSearch] transport={} returned {}; retrying direct",
                    route.as_str(),
                    response.status()
                );
                continue;
