...
use super::types::{
    CitationSource, SearchBackend, SearchError, SearchFailureClass, TransportRoute, WebSearchResult,
};
...
pub(crate) async fn run_backend_query_once(
    ...
    let direct_retry_url = search_url.clone();
    ...
    let mut html = read_capped_response_body(response, MAX_FETCH_BYTES).await?;
    let challenged = |body: &str| match backend {
        SearchBackend::DuckDuckGo => looks_like_ddg_challenge_page(body),
        SearchBackend::Mojeek => looks_like_mojeek_block_page(body),
    };
    if route == TransportRoute::Proxy && challenged(&html) {
        if let Some(direct) = clients.client_for(TransportRoute::Direct) {
            if let Ok(response) = direct
                .get(direct_retry_url)
                .header(header::ACCEPT, "text/html")
                .send()
                .await
            {
                if response.status().is_success() {
                    if let Ok(body) = read_capped_response_body(response, MAX_FETCH_BYTES).await {
                        html = body;
                    }
                }
            }
    ...
    let parsed = match backend {
        SearchBackend::DuckDuckGo => parse_ddg_results(&html, max_results.saturating_mul(4)),
        SearchBackend::Mojeek => parse_mojeek_results(&html, max_results.saturating_mul(4)),
    };
