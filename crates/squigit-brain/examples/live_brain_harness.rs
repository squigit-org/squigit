...
use squigit_auth::security::{encrypt_and_save_api_key, validate_api_key, ApiKeyProvider};
...
async fn run() -> Result<(), String> {
    ...
    match command.as_str() {
        "analyze" => {
            ...
            let profile = prepare_live_profile(Some(&provider_api_key()?))?;
        ...
        "prompt" => {
            ...
            let result = BrainService::new()
                .prompt_thread(
                    ...
                    PromptThreadRequest {
                        profile_id: profile.id,
                        main_model_candidates: model_candidates,
                        thread_id,
...
mod tests {
    ...
    fn profile_bootstrap_is_idempotent_and_encrypts_the_key() {
        ...
        assert_eq!(
            reveal_api_key(&store, ApiKeyProvider::GoogleAiStudio, &first.id)
                .unwrap()
                .as_ref()
