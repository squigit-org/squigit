...
/// Resolve the installed application's config root without honoring live/test overrides.
pub fn default_app_config_dir() -> Option<PathBuf> {
    dirs::config_dir().map(|path| path.join(APP_DIR_NAME))
}

/// Return whether a candidate resolves to the installed application's config root.
pub fn is_default_app_config_dir(candidate: &Path) -> bool {
    let Some(app_dir) = default_app_config_dir() else {
        return false;
    };
    match (candidate.canonicalize(), app_dir.canonicalize()) {
        (Ok(candidate), Ok(app_dir)) => candidate == app_dir,
        _ => candidate == app_dir,
    }
}
