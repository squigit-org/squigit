...
use squigit_auth::security::{
    check_reveal_authorization, encrypt_and_save_api_key, reveal_api_key, ApiKeyProvider,
...
fn run() -> Result<(), String> {
    ...
    match command.as_str() {
        ...
        "save-key" => {
            ...
            encrypt_and_save_api_key(&store, &profile_id, provider, &plaintext)
                .map_err(|err| err.to_string())?;
            println!("ok");
        }
        "get-key" => {
            ...
            store
                ...
                .map_err(|err| err.to_string())?;
            if let Some(value) =
                reveal_api_key(&store, provider, &profile_id).map_err(|err| err.to_string())?
            {
...
fn prompt_terminal_pin() -> Result<(), String> {
    ...
    let entered = rpassword::prompt_password("PIN: ")
        .map_err(|err| format!("Could not read terminal PIN: {err}"))?;
    if entered.trim() != pin {
