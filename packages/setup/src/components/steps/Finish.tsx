...
export const Finish: React.FC<Props> = ({ launchOnExit, setLaunchOnExit, onFinish }) => {
  return (
    <StepLayout
      ...
      onNext={onFinish}
    >
      <div className="flex h-full -m-5"> {/* Negative margin to offset padding from StepLayout content area */}
        {/* Left Side - Image */}
        ...
        <div className="flex-1 px-8 pb-8 pt-20 flex flex-col bg-white">
            ...
            <div className="flex-1"></div> {/* Spacer to push content up if needed, or just let it sit top */}
        </div>
      </div>
    </StepLayout>
  );
