...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onComplete }) => {
  ...
  return (
    <div className={layoutStyles.viewContainer}>
      <div
        className={`${layoutStyles.container} ${layoutStyles.containerLogin}`}
      >
        <h1 className={typoStyles.h1}>Welcome Back</h1>
        <p className={typoStyles.p}>Please sign in to continue.</p>
        <button
          onClick={handleClick}
        ...
        >
          ...
          <span>
            {isLoading ? "Check your browser..." : "Continue with Google"}
          </span>
          <div className={buttonStyles.btnBorder}></div>
        </button>
      </div>
      <div className={layoutStyles.footer}>Secure login powered by Google.</div>
    </div>
  );
};
