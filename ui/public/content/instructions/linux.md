...
SnapLLM is designed as a self-contained, XDG-compliant application with native Wayland support via Desktop Portals.
...
- **Manual Setup:** If the hotkey does not trigger:
  ...
  3. **Command:** Point to the daemon binary at `~/.local/share/snapllm/daemon`.
...
- **On-Demand Execution:** SnapLLM does not run a persistent background daemon on Linux. The shortcut triggers the application directly via your Compositor (Mutter/KWin), ensuring zero idle resource usage.
